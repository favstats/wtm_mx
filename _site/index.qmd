...
out <- sets$cntry %>% 
  ...
  unlist() %>% 
  keep(~str_detect(.x,"lifelong")) %>%# bashR::simule_map(1)
  # .[100:120] %>% 
  map_dfr_progress(~{
...
retrieve_reports <- function(fiiilet) {
  thetemps <- tempfile()
# fiiilet <- gv$filename[2]
download.file(paste0("https://github.com/favstats/meta_ad_reports/releases/download/MX-lifelong/", fiiilet), thetemps, quiet = T)
fin <- read_rds(thetemps)
file.remove(thetemps)
return(fin)
}

phil <- gv %>% 
  filter(str_detect(filename, "2024")) %>% 
  pull(filename) %>% 
  map_dfr_progress(retrieve_reports)

phil %>% 
  arrange(page_id, date) %>% 
  mutate(amount_spent = parse_number(amount_spent_mxn)) %>% 
  group_by(page_id, date) %>% 
  summarize(amount_spent = sum(amount_spent)) %>% 
  mutate(lagged_var = (lag(amount_spent))) %>% 
  mutate(lagged_dat = amount_spent -lagged_var) %>% 
  group_by(date) %>% 
  summarize(lagged_dat = sum(lagged_dat, na.rm = T)) %>% 
  mutate(date = lubridate::ymd(date)) %>% 
  ggplot(aes(date, lagged_dat)) +
  geom_smooth()
